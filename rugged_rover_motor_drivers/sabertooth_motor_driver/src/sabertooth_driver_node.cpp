...
namespace sabertooth_motor_driver
{
  ...
  SabertoothDriverNode::SabertoothDriverNode(const std::string& node_name) : Node(node_name)
  {
    ...
    feedback_timer_ =
        this->create_wall_timer(std::chrono::duration_cast<std::chrono::milliseconds>(interval),
                                std::bind(&SabertoothDriverNode::feedbackTimerCallback, this));
  };
  ...
  void SabertoothDriverNode::cmdCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
  {
    ...
    RCLCPP_DEBUG(this->get_logger(), "Received velocities - Left: %.2f, Right: %.2f", left_vel,
                 right_vel);
