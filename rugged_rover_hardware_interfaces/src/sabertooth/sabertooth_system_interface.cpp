 ...
 * returns an error.
 */
hardware_interface::CallbackReturn SabertoothSystemInterface::on_init(
...
SabertoothSystemInterface::on_activate(const rclcpp_lifecycle::State &) {
  ...
  feedback_sub_ = node_->create_subscription<sensor_msgs::msg::JointState>(
      ...
      std::bind(&SabertoothSystemInterface::feedbackCallback, this,
                std::placeholders::_1));
...
SabertoothSystemInterface::read(const rclcpp::Time &,
  ...
  for (size_t i = 0; i < joint_names_.size(); ++i) {
    ...
    auto it = std::find(last_feedback_.name.begin(), last_feedback_.name.end(),
                        joint_names_[i]);
 ...
 */
hardware_interface::return_type
SabertoothSystemInterface::write(const rclcpp::Time &,
                                 const rclcpp::Duration &) {
  // Check if the command publisher and node are initialized
  if (!cmd_pub_ || !node_) {
    RCLCPP_ERROR(this->logger_,
                 "Command publisher or node is not initialized.");
